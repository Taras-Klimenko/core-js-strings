...
function isPalindrome(str) {
  const normalizedStr = str
    ...
    .toLowerCase()
    .replace(/[^a-z]/g, '');
  for (let i = 0; i <= Math.floor(normalizedStr.length / 2); i += 1) {
...
function extractNameFromTemplate(value) {
  ...
  return `${wordsArray[1]} ${wordsArray[2].slice(0, wordsArray[2].length - 1)}`;
}
